...
public sealed class TelemetryGraph : Control
{
    ...
    public static readonly StyledProperty<IBrush?> GridStrokeProperty = AvaloniaProperty.Register<TelemetryGraph, IBrush?>(nameof(GridStroke));
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
    {
        ...
        if (change.Property == ValuesProperty || change.Property == StrokeProperty || change.Property == GridStrokeProperty || change.Property == MinimumProperty || change.Property == MaximumProperty || change.Property == ShowGridProperty) InvalidateVisual();
    }
    ...
    public override void Render(DrawingContext context)
    {
        ...
        if (ShowGrid)
        {
            var gridPen = new Pen(GridStroke ?? new SolidColorBrush(Color.FromArgb(55, 57, 91, 100)), 1);
        ...
        {
            var y = Math.Clamp(height - Math.Clamp((validValues[0] - min) / (max - min), 0, 1) * height, 2, height - 2);
        ...
        for (var index = 1; index < validValues.Length; index++)
        {
            ...
            var y1 = Math.Clamp(height - Math.Clamp((validValues[index - 1] - min) / (max - min), 0, 1) * height, 2, height - 2);
            var y2 = Math.Clamp(height - Math.Clamp((validValues[index] - min) / (max - min), 0, 1) * height, 2, height - 2);
